...
- `client/supervisor.py` — CLI (`screencrop-supervisorctl`, alias `screencrop-supervisor-worker`) to operate a fleet of host `screencrop-worker` processes (competing consumers on one shared queue). Pure core (`worker_specs`, `WorkerState` + `write_state`/`read_states`, `is_alive`, `build_status_rows`, `tail_command`) is torch-free and unit-tested under `tmp_path`; thin IO wrappers (`spawn_worker`, `signal_worker`, `run_logs`, `probe_worker`) are patched in tests. `start`/`stop`/`restart`/`status`/`logs` assign each worker a `base+i` metrics port + `logs/supervisor/worker-<i>.{log,json}`, reuse `serve.resolve_serve_weights` for `--select`/`--fuzzy`, and shell out to `tail`/`less -R` for `logs`. Paired with the worker's SIGTERM warm-shutdown (`worker.drain_and_cancel`). See `docs/supervisor.md`.
