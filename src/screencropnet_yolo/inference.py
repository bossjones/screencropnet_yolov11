...
class InferencePipeline:
    ...
    def predict_image(
        ...
        result = InferenceResult(
            image_path=image_path, image_size=img_size, inference_time=inference_time
        )
    ...
    def draw_detections(
        ...
        for det in result.detections:
            ...
            # Label background
            cv2.rectangle(
                annotated, (x1, y1 - label_size[1] - 10), (x1 + label_size[0], y1), color, -1
            )
...
class ResultExporter:
    ...
    @staticmethod
    def to_json(results: InferenceResult | list[InferenceResult], output_path: str) -> str:
        """
    ...
    @staticmethod
    def to_coco(results: list[InferenceResult], output_path: str, class_names: list[str]) -> str:
        """
    ...
    @staticmethod
    def to_yolo(results: list[InferenceResult], output_dir: str) -> str:
        """
        ...
        for result in results:
            ...
            with open(txt_path, "w") as f:
                for det in result.detections:
                    ...
                    f.write(
                        f"{det.class_id} {x_center:.6f} {y_center:.6f} {width:.6f} {height:.6f}\n"
                    )
...
def apply_nms(
    detections: list[Detection], iou_threshold: float = 0.45, class_agnostic: bool = False
) -> list[Detection]:
    """
